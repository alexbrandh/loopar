...
export default function GradientMenu({ className }: GradientMenuProps) {
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 z-50 safe-area-bottom pointer-events-none ${className}`}>
      <div className="flex justify-center items-center py-4 px-4 pointer-events-auto">
        <ul className="flex gap-2 sm:gap-3">
          {menuItems.map(({ title, icon, href, gradientFrom, gradientTo, requiresAuth }, idx) => {
            ...
            const MenuItem = (
              <li
                ...
                className={`relative w-[32px] h-[32px] sm:w-[36px] sm:h-[36px] bg-card/80 backdrop-blur-sm shadow-sm rounded-full flex items-center justify-center transition-all duration-500 hover:w-[75px] sm:hover:w-[85px] hover:shadow-none group cursor-pointer ${isActive ? 'w-[75px] sm:w-[85px] shadow-none' : ''}`}
              >
                ...
                {/* Blur glow */}
                <span className={`absolute top-[8px] inset-x-0 h-full rounded-full bg-[linear-gradient(45deg,var(--gradient-from),var(--gradient-to))] blur-[12px] -z-10 transition-all duration-500 ${isActive ? 'opacity-40' : 'opacity-0 group-hover:opacity-40'}`}></span>
          ...
          <SignedOut>
            <SignInButton mode="modal">
              <li
                ...
                className="relative w-[32px] h-[32px] sm:w-[36px] sm:h-[36px] bg-card/80 backdrop-blur-sm shadow-sm rounded-full flex items-center justify-center transition-all duration-500 hover:w-[75px] sm:hover:w-[85px] hover:shadow-none group cursor-pointer"
              >
          ...
          <SignedIn>
            <li className="relative w-[32px] h-[32px] sm:w-[36px] sm:h-[36px] bg-card/80 backdrop-blur-sm shadow-sm rounded-full flex items-center justify-center">
              <UserButton 
                appearance={{
                  elements: {
          ...
          <li
            ...
            className="relative w-[32px] h-[32px] sm:w-[36px] sm:h-[36px] bg-card/80 backdrop-blur-sm shadow-sm rounded-full flex items-center justify-center transition-all duration-500 hover:w-[65px] sm:hover:w-[70px] hover:shadow-none group cursor-pointer"
          >
            <span className="absolute inset-0 rounded-full bg-[linear-gradient(45deg,var(--gradient-from),var(--gradient-to))] opacity-0 transition-all duration-500 group-hover:opacity-100"></span>
